...
- 报告：acceptance-darwin-20260921-150201.md
- 生成时间：20260921-150201
...
白名单字段（wrdKey/wrdIv 不写入报告）：
{
  ...
  "settings": {
    ...
    "captureProcesses": [
      ...
      "/Users/m2/.omp/puppeteer/chrome/mac_arm-150.0.7871.24/chrome-mac-arm64/Google Chrome for Testing.app/"
    ],
...
```text
$ curl -sS -x http://127.0.0.1:8080 --cacert <caCert> -m 20 -o /dev/null -D - http://jwxt.swufe.edu.cn/
exit: 0
stdout:
(空)
stderr:
...
响应头（已脱敏）：
HTTP/1.1 200 OK
Date: Mon, 21 Sep 2026 07:02:02 GMT
Content-Type: text/html;charset=utf-8
Content-Length: 925
...
exit: 0
stdout:
58285 /Volumes/西数2T/Chen/swufe-webvpn-codec/.venv/bin/python -m swufe_bridge.sidecar --config /tmp/m4-acceptance/bridge-config.json --port 8080 --confdir /tmp/m4-acceptance/mitmproxy
stderr:
(空)
```
