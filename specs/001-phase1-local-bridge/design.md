...
4. **响应反向改写**（按优先级）：`Location` → `Set-Cookie` 的 Domain/Path → `text/html`/`application/javascript`/`application/json` 中的绝对 URL → 其它内容类型不改写。策略固定为「客户端侧始终使用真实主机名，仅上行改走 WebVPN」，两条例外（[ADR-0007](../../docs/architecture/adr/ADR-0007-gateway-owned-namespaces-and-native-mode-promotion.md)，`KI-011`）：网关自有根命名空间（`/wengine-vpn/`、`/authserver/`）不经 token、直接取自网关根，且其响应不反向改写；改写过的 HTML 若命中网关 bootstrap 判据（`text/html` + ≤ 8192 B + 含 `__vpn_` 与 `/wengine-vpn/js/main.js`）则以 `302` 升级到该 URL 的 WRD 形态，此后该主机在网关原生 URL 空间内由网关自己的改写运行时接管，其它 allowlist 主机仍留在普通 URL 空间。
...
说明：命中 allowlist 的请求只在上行改为 WebVPN 形态；响应反向改写后浏览器地址栏与页面链接仍是普通主机名，从而再次命中本桥。例外见 [ADR-0007](../../docs/architecture/adr/ADR-0007-gateway-owned-namespaces-and-native-mode-promotion.md)：网关自有根命名空间（`/wengine-vpn/`、`/authserver/`）不经 token 直接取自网关根；命中网关 bootstrap 判据的 HTML 文档以 `302` 升级到网关原生 URL 空间，该主机此后的地址栏为 `https://webvpn.swufe.edu.cn/<scheme>/<token>/…`（桥不参与这些页面的改写）。未命中主机不经过 WRD 改写，保持直连语义。
...
| 是否需要 ADR | 需要（已完成） | 七个难以逆转的决策已在架构层决定并 Accepted：[ADR-0001](../../docs/architecture/adr/ADR-0001-wrd-rewrite-in-mitm-layer.md)、[ADR-0002](../../docs/architecture/adr/ADR-0002-reuse-mitmproxy-for-tls.md)、[ADR-0003](../../docs/architecture/adr/ADR-0003-electron-gui-for-phase-1.md)、[ADR-0004](../../docs/architecture/adr/ADR-0004-refuse-start-when-system-proxy-in-use.md)、[ADR-0005](../../docs/architecture/adr/ADR-0005-builtin-wrd-key-with-override.md)、[ADR-0006](../../docs/architecture/adr/ADR-0006-local-capture-mode-and-mutual-exclusion.md)（2026-09-21，进程捕获与系统代理互斥）、[ADR-0007](../../docs/architecture/adr/ADR-0007-gateway-owned-namespaces-and-native-mode-promotion.md)（2026-09-21，网关自有命名空间直通 + bootstrap 文档升级） | ADR-0001..ADR-0007 |
...
| Bridge Addon | 新增 | 请求改写、响应反向改写、Cookie 注入、直连判定；网关自有根命名空间（`/wengine-vpn/`、`/authserver/`）不经 token 直接取自网关根（其响应不反向改写）；命中网关 bootstrap 判据的 HTML 文档以 `302` 升级到网关原生 URL 空间（[ADR-0007](../../docs/architecture/adr/ADR-0007-gateway-owned-namespaces-and-native-mode-promotion.md)）；把运行时模式切为 `regular@<port>,local:<spec>` 实现进程捕获并上报 `swufe-capture` |
...
- **请求流**：本机浏览器/所选应用 → （`system-proxy`：系统代理；`selected-apps`：local 模式按进程接管，两者互斥）→ Bridge Addon → allowlist 匹配 → 命中：网关自有根命名空间（`/wengine-vpn/`、`/authserver/`）直接取自网关根，其余 WrdCodec 编码 + Cookie → `webvpn.swufe.edu.cn`；未命中：直连。失败路径：sidecar 退出 ⇒ `BRIDGE_CRASH`（UI 提示重启桥/查看日志）。
- **响应流**：上游响应 → Addon 反向改写（Location → Set-Cookie Domain/Path → HTML/JS/JSON 绝对 URL）→ 浏览器；网关自有命名空间的响应原样透传；命中 bootstrap 判据的 HTML 以 `302` 升级到网关原生 URL 空间（[ADR-0007](../../docs/architecture/adr/ADR-0007-gateway-owned-namespaces-and-native-mode-promotion.md)）。失败路径：内容类型不在改写列表 ⇒ 原样透传（记为未改写）。
