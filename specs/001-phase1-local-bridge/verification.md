...
## KI-019 最新试验与临时风险接受（2026-09-23）

> 最新试验对应上一节的四臂配对复测：应用桥、同桥中性头、stock mitmdump 与直连共 80 轮，均未观察到不可达或挂起。该轮次未出现历史爆发窗口，按原机械判据仍不足以判断根因；本次状态决策接受当前残余风险，不把未复现解释为历史问题已排除，也不覆盖此前 Windows 19/40 轮 `bridge-side` 命中记录。

**决策**：`KI-019` 暂列 `Accepted`，含义是暂时接受当前残余风险，不代表已证明根因在桥外或问题已修复。上游建连已有 4s × 2 次有界失败与阶段记录；已建连后不返回数据仍可能持续等待。若后续出现不可达或挂起，需保留同轮桥路与直连结果及阶段记录：桥路单独失败则重开 `KI-019` 并继续定位；两路均失败则按外部网络风险重新评估。Spec 001 仍保持 `Implemented`，本次只调整已知问题风险状态。
...
> 当前：macOS 与 Windows 的既定 P0 用例和教务浏览器验收均已通过（Windows 2026-09-23 实测，`KI-001` 已 `Fixed`）；本轮将 CAS 资源截断 `KI-014` 置 `Accepted`，重载登录窗为现有规避。教务请求偶发挂起 `KI-019` 暂列 `Accepted` 作为残余风险：最新试验桥路与直连均未观察到不可达，但根因未知、复现时重开。
