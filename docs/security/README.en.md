...
| Uninstall the MITM CA | TLS interception fails if the bridge is still running; another tool may own a CA with the same name | Explicit local user action; stopping the bridge first is recommended; identify and delete only this app's local CA by its SHA-1 fingerprint, never by the shared name |
...
| Clear the system proxy | Accidentally clearing the user's proxy, or leaving this bridge's proxy after failed cleanup | Clear only proxies pointing at this bridge's address and port (NFR-004); persist ownership before the OS write, clear the marker only after successful cleanup, and retry on the next launch after failure |
...
Stash/Loon plugins reuse the host Network Extension, HTTP Engine, MitM CA, and Script. M2 stores only the webvpn-gateway Gateway Session. With safe classification, the proxy layer may reuse it on another App/WKWebView Gateway request, and it is sent only to webvpn.swufe.edu.cn. Ordinary origins, raw authserver requests, and the Settings Namespace never receive it. A Gateway ticket supplied by the client is preserved and captured/refreshed; an older stored ticket never replaces it. Safari/WKWebView Cookie Jars are neither shared nor modified. CAS Cookies are not captured, stored, or injected in the current scope.

Safe Auth Trace records only locally allowlisted fields: timestamp, host/path classification, route/request kind, ticket/session state, injection and client precedence, WRD original host, redirect host, necessary Cookie names, and the CAS service target hostname. It must omit Cookie values, CAS tickets, execution, Authorization, credentials/MFA, full queries/service URLs, long WRD tokens, and request/response bodies. Trace never enters Settings, notifications, cloud services, or the Session Store. See [ADR-0015](../architecture/adr/ADR-0015-session-realm-and-proxy-reuse.en.md) and [Spec 003 interfaces](../../specs/003-ios-proxy-client-plugins/interfaces.md).
