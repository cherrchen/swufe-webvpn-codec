...
| [M4 acceptance](milestones/M4-acceptance.md) | macOS + Windows academic-affairs browser acceptance, defect burn-down | G-001 / G-002 / G-003 | TBD | In Progress (2026-09-21: on macOS, including the academic-affairs browser acceptance, everything **now passes** - after the `KI-011` fix the M5 re-verification passed TC-G01/G02; **2026-09-23: every Windows real-machine item passed on a real Windows 11 24H2 machine** (including the TC-G03 browser acceptance and the real TC-G04 process capture), `KI-001` is `Fixed`, four Windows-only defects were fixed (`KI-015`..`KI-018`) plus `KI-020`, and `KI-019` was accepted by decision; the only remaining exit condition is the decision on `KI-014`) |
...
| ---- | ----- | ----------- | ----- | ------ | ---- |
| 001-phase1-local-bridge | Phase 1 local bridge and academic-affairs browser acceptance | REQ-001..REQ-011, NFR-001..NFR-007 | M1–M4 | Implemented (2026-09-23: the P0 cases and the browser acceptance pass on both macOS and Windows - see "M4 Windows 验收执行记录" - with `KI-001` `Fixed` and `KI-019` accepted; `Verified` only awaits the decision on `KI-014`) | [spec.md](../../specs/001-phase1-local-bridge/spec.md) |
...
| M4 | M3 + one macOS and one Windows test machine + the tester's own SWUFE test account | The dual-platform P0 cases and the browser acceptance need real machines, the real WebVPN and an account | **Lifted (2026-09-23)**: the Windows machine and account were available and every Windows real-machine item ran and passed (`KI-001` set to `Fixed`) |
