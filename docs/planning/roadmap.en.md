...
| [M4 acceptance](milestones/M4-acceptance.md) | macOS + Windows academic-affairs browser acceptance, defect burn-down | G-001 / G-002 / G-003 | TBD | In Progress (P0 and browser acceptance pass on both OSes; `KI-014` is accepted as external transfer risk; `KI-019` is reopened because the upstream attribution remains unproven) |
...
| ---- | ----- | ----------- | ----- | ------ | ---- |
| 001-phase1-local-bridge | Phase 1 local bridge and academic-affairs browser acceptance | REQ-001..REQ-011, NFR-001..NFR-007 | M1–M4 | Implemented (real-machine acceptance passes on both OSes and `KI-014` is `Accepted`; `KI-019` must be isolated from local causes or fixed before `Verified` is assessed) | [spec.md](../../specs/001-phase1-local-bridge/spec.md) |
