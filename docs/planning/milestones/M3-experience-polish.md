...
- [x] 相关文档已同步（含双语配对，`npm run docs:check` 0 error/0 warning）；无阻塞类缺陷
...
- **完成时间**：2026-09-21
- **交付物**
  - **捕获方式（一级）与进程捕获**：`app/static/index.html` 捕获方式区（系统代理 / 指定应用 + 进程捕获状态 + 应用列表 + macOS 授权引导 + 重试）；`app/src/renderer/renderer.ts`（`renderCapture`、`applyCaptureMode`、失败态与筛选）；`app/src/shared/types.ts`（`CaptureMode` / `CaptureCandidate.pattern` / `CaptureReport` / `BridgeStatus.captureError`）；`app/src/main/{store,orchestrator,ipc,sidecar,constants}.ts`（`captureMode` / `captureProcesses` 持久化与校验、两种方式的互斥与撤销/恢复系统代理、`swufe-capture` 解析与状态上报）；`app/src/main/platform/parse.ts`（`capturePattern` / `captureName` / `groupCaptureCandidates`，一个应用一行）；`swufe_bridge/capture.py`（模式集推导、失败回滚、同配置不重试）+ `swufe_bridge/addon.py`（捕获循环与 `swufe-capture` 回报）+ `swufe_bridge/sidecar.py`（`--mode regular@<port>`）+ `swufe_bridge/config.py`（`capture.processes` 与 `ConfigWatcher.stamp`）。
...
- **验证证据**（完整命令与观察结果见 [verification.md](../../../specs/001-phase1-local-bridge/verification.md)）
  - `uv run pytest -q` → `190 passed`（L0 97 / L1 86 / L2 7；M3 新增 `tests/l0/test_capture.py` 14 例、`tests/l1/test_addon_capture.py` 7 例、config 捕获用例 7 例、L2 捕获用例 2 例）。
  - `npm --prefix app run typecheck` 无 error；`npm --prefix app run test:unit` → `70 passed`；`npm --prefix app run build` 通过。
  - `npm run docs:check` → `0 error(s), 0 warning(s)`（148 文件）；`npm run typecheck` 无 error。
...
- **实现期决策与偏差**（均已落文档，不再重复决策）
  ...
  5. **偏差（实现细节）**：`capturePattern` 的正则用**非贪婪** `^(.*?\.app)/Contents/`（计划文本写的是贪婪版），否则 Chrome Helper 会被归并到 `…/Google Chrome Helper.app/` 而不是外层 `Google Chrome.app/`，与「主进程与 Helper 合并为一行」的目标冲突；已由 `app/test/system-proxy-parse.test.ts` 固定。
