...
**描述**
系统应当提供 macOS / Windows 桌面窗口（托盘非必须），并具备以下界面能力：登录（内嵌 BrowserWindow/WebView 打开官方 WebVPN/CAS，用户自行完成认证，含 MFA）、一键连接开关、allowlist 管理（查看/增删主机，默认含 `jwxt.swufe.edu.cn`，可选一键启用 `*.swufe.edu.cn`）、状态区（已连接 / 已断开 / 错误原因，以及当前 allowlist 摘要）、证书（一键安装、一键卸载本机 MITM 根证书）、调试日志开关。
...
2. 状态条与桥接状态机一致：未登录为灰色且开关禁用；已登录未开桥为蓝色「已登录」且可开；桥接中为绿色「桥接中」且可关；错误为红色并显示原因；过期处理中为橙色且强制关（TC-H01）。
...
**描述**
系统应当同时支持两种流量接管方式：系统 HTTP/HTTPS 代理指向本地桥端口；mitmproxy local（或等价）按进程捕获，便于点选浏览器等进程。关闭开关时必须停止代理服务、恢复/清除系统代理、停止进程捕获。

**理由**
第一期不做 TUN/透明网关，系统代理可覆盖普通浏览器，进程捕获作为补充手段，两者可同时启用。

**验收标准**
1. 已登录、系统无代理、CA 就绪时开桥，系统 HTTP/HTTPS 代理指向本桥端口（TC-C02）。
2. 关闭开关后，系统代理恢复为不再由本 App 占用（TC-C03）。
3. 可列出候选进程并选择需要捕获的进程（如 Chrome），关闭开关时进程捕获一并停止。
4. CA 未安装时打开开关，返回 `CA_MISSING` 或给出明确失败提示（TC-E03）。

**边界与例外**
- macOS 下进程捕获可能触发辅助功能/网络扩展授权，UI 须给出引导。
...
**描述**
系统应当默认可见连接状态、allowlist 与错误原因；提供可开关的调试日志，日志内容仅为「域名 + 是否改写成功」，默认关闭，且不记录响应正文、请求体与 Cookie。
