...
- `swufe-upstream` (`KI-019`, 2026-09-23) carries **upstream stage evidence**: exactly six keys — `ts` / `stage` / `host` / `addr` / `ms` / `detail`. Electron does **not** parse this line (`parseSidecarLine` recognises only the four other lines plus `swufe-error`), and it never gates readiness.
  - `stage` vocabulary: `connect_start`, `connect_done`, `connect_timeout`, `connect_retry`, `connect_failed`, `tls_start`, `tls_done`, `tls_failed`, `response`, `error`.
  - `ms` semantics: `connect_done` = TCP connect time including the DNS lookup; `tls_done` = handshake time counted from that connection's TCP setup; `response` = time to the first upstream byte; `connect_timeout` / `connect_retry` = the wait of that attempt; everything else is `null`.
  - Trigger (any one of the three; otherwise nothing is written at all): `debug=true`; `stage` ∈ {`connect_timeout`, `connect_retry`, `connect_failed`, `tls_failed`, `error`}; `ms >= 3000`. The normal path writes zero records.
  - Sink: the same line goes to stderr and is appended to `<userData>/bridge-upstream.log` (JSONL, 256 KiB cap, newest ~half kept on overflow, `0600` on POSIX).
  - Redaction (INV-001): `detail` first drops every `http(s)://` run up to the next whitespace or quote (a WRD token only ever appears inside a URL), then flattens control characters and truncates to 160 characters; `host` / `addr` accept a hostname or `host:port` only and are recorded as `null` otherwise. Neither the line nor the file carries cookie values, tokens or bodies.
  - Bounded connects: for the gateway host the bridge bounds the upstream connect to 4 seconds × 2 attempts (`CONNECT_TIMEOUT_SECONDS` / `CONNECT_ATTEMPTS` in `swufe_bridge/upstream.py`, installed by the sidecar before `mitmdump()`). After both attempts time out the existing mitmproxy failure path applies, so the client gets `502 Bad Gateway` instead of waiting forever; requests whose upstream is not the gateway host (including other sites) are unaffected. **Known limitation**: once connected, an upstream that never answers cannot be interrupted (mitmproxy's `flow.kill()` does not abort in-flight flows); it is only made visible through the `response` / `error` records.
...
| 2026-09-23 | `KI-019` bounding: new `swufe-upstream` diagnostic line (six keys, ten stages, `ms` semantics and trigger) plus `<userData>/bridge-upstream.log` (JSONL, 256 KiB cap); upstream connects to the gateway host become 4 s × 2 attempts (a timeout reaches the client as 502); `swufe-ready` gains `upstream_connect_timeout_ms` / `upstream_connect_attempts` / `upstream_log` | Compatible (new line and keys; the existing `swufe-debug` / `swufe-capture` / `swufe-ready` keys are unchanged and Electron ignores unknown lines) | [spec 001](../../specs/001-phase1-local-bridge/known-issues.md) |
