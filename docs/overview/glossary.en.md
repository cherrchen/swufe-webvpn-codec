...
| 防环 | loop prevention | Raw requests to webvpn.swufe.edu.cn and authserver.swufe.edu.cn are not wrapped as ordinary targets. Spec 003 may reuse a Gateway Session only on classified direct Gateway requests; unknown, login, logout, callback, Settings, ordinary-source and raw authserver requests do not receive stored Gateway Cookies by default | Not to be confused with native Cookie Jar sharing or ordinary WRD routing | ADR-0007 for desktop namespace behavior; Spec 003 / ADR-0015 for mobile classification |
| Session Realm | Session Realm | An authentication context with its own host boundary, storage and lifecycle. Spec 003 defines webvpn-gateway now and cas-sso only as a separate future option | Do not merge the two Cookie sets | Spec 003 domain model; ADR-0015 |
| Gateway Session | Gateway Session | Ticket and minimal Cookie state for webvpn.swufe.edu.cn; captured by the proxy and usable only on Gateway requests | Not a CAS/SSO Session | Spec 003 / ADR-0015 |
| CAS Session | CAS/SSO Session | Authentication session for authserver.swufe.edu.cn. It is not captured, stored or injected in the current plugin scope | Not a Gateway Session | Spec 003 / ADR-0015 |
| Browser Cookie Jar | Browser Cookie Jar | Cookies managed independently by Safari, a WKWebView, or another App | Not the Plugin Gateway Session Store | Spec 003 / ADR-0015 |
| Inter-App Gateway Session Reuse | Inter-App Gateway Session Reuse | Proxy-layer reuse of the locally stored Gateway Session on a safely classified Gateway request from another client App; it does not share Cookie Jars | Not CAS Session Bridge | Spec 003 / ADR-0015 |
