...
App unit:       pnpm --filter swufe-webvpn-bridge run test:unit（首次先 `pnpm install`）
...
CI test job:    L0：.github/workflows/python-tests.yml（pull_request 与 main 推送时执行 uv sync --frozen --directory bridges/python + uv run --directory bridges/python pytest tests/l0 -q）；
                App 单元与类型：.github/workflows/app-tests.yml（pnpm install --frozen-lockfile + typecheck + test:unit，不需要 Electron 二进制与显示器）；
