...
| L2.5 renderer components | **observable UI behaviour** of the four windows' renderer (copy, disabled rules, calls and arguments, event-driven state updates, empty and failure states, modal paths); state is injected through a fake `window.swufeBridge`, with neither Electron nor the sidecar running | `apps/desktop/src/renderer/**` (components, hooks, log batching) + jsdom | low | every PR (same batch as the app unit tests); whenever renderer component behaviour changes | CI |
...
**Regression policy**: L0 runs on every PR; L1/L2 run locally when the relevant module changes; L2.5 runs on every PR in the same batch as the app unit tests; L3 runs before a release. The case set (TC-A01..TC-H02) and priorities live in the phase 1 spec's `verification.md`; the release gate is section 6 of [docs/verification/verification-strategy.md](../verification/verification-strategy.md).

**Test boundaries for the renderer (M6)**: L2.5 asserts **observable UI behaviour** only and does not replace real-machine verification. The following are **never asserted in jsdom** and always go through CDP against the running app (`pnpm start --user-data-dir=… --remote-debugging-port=9222`, criteria in spec 002's `verification.md`): **main-window zero scrolling** (`scrollHeight <= clientHeight` with no scrollable overflowing element), **window count and single-instance reuse**, **window open latency** (≤ 500ms from trigger to `load`), and **whether the CSP is in force** (the meta tag in the built HTML plus no real-machine violations). jsdom has no real window/compositor and no real `file://` CSP behaviour, so those conclusions are not trustworthy there.
...
- observable renderer behaviour (L2.5, M6): the four windows' fixed copy, disabled rules, IPC calls and arguments, event-driven state updates, empty and failure states, modal paths (proxy conflict / CA risk notice / session expiry), log batching and the 200-entry cap, and history recovery after a window is reopened; state authority is still covered by the real-machine items (see the test boundaries in section 1).
...
Location:  bridges/python/tests/l0 (unit: codec, allowlist, config), bridges/python/tests/l1 (component: addon request/response
           ...
           apps/desktop/test/ui (renderer components: `*.test.tsx` + `helpers/{setup.ts,bridge-fake.ts,render.tsx}`, vitest + jsdom;
           `bridge-fake.ts` provides a controllable fake window.swufeBridge — it records calls and arguments and can emit onStatus/onDebugLog/onSessionExpired)
Naming:    files test_<topic>.py; functions test_<behaviour> (the case id goes into the function name,
           ...
           on the app side files are <topic>.test.ts and renderer component files <topic>.test.tsx, with case names as full sentences describing observable behaviour
...
App unit:       pnpm --filter swufe-webvpn-bridge run test:unit (run `pnpm install` first; node:test + tsx)
App UI:         pnpm --filter swufe-webvpn-bridge run test:ui (vitest + jsdom; starts neither Electron nor the sidecar and needs no display)
...
CI test job:    L0: .github/workflows/python-tests.yml (runs uv sync --frozen --directory bridges/python + uv run --directory bridges/python pytest tests/l0 -q
                ...
                app unit tests, renderer components and types: .github/workflows/app-tests.yml (pnpm install --frozen-lockfile + typecheck + test:unit + test:ui,
