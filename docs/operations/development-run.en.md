...
- The script stops with a clear error — and no conclusion — when the bridge is not running (port unreachable in three consecutive rounds), the session has expired (same-path direct control `302 → /login`), or `bridge-config.json` / the CA is missing from `--user-data-dir`.
