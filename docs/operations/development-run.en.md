...
| After clicking "install local CA" the UI reports "命令超时（10000ms）：certutil" (Windows) | Before writing a root certificate into the current user's store, Windows raises the system "security warning" dialog (it lists the certificate fingerprint and waits for 是(Y) / 否(N)), and the `certutil` child process blocks meanwhile; the app's timeout for that step is 10 s (`KI-021`) | Click 是(Y) within 10 s of the dialog appearing; if it already timed out, run the same command by hand: `certutil -user -addstore Root <userData>\mitmproxy\mitmproxy-ca-cert.cer` (it shows the same dialog; no administrator rights needed), then the UI refreshes to "已安装并被系统信任" |
