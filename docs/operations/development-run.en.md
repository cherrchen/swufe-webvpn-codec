...
| The registrar page shows nothing for tens of seconds on first open | Sporadic bridged-request stall (`KI-019`): the bridge logged the request, but the stall has not been isolated between local connection handling, the network, and the gateway; the old direct control used the gateway root rather than the same path | Reload the page (or close and reopen it). The same symptom shows up as `curl 退出码 28` in an acceptance smoke run — just re-run it; reproduction data is recorded in `KI-019` |
