...
> On Windows, CA installation writes to the **current-user** Root store (`certutil -user -addstore Root <caCert>`) and shows no administrator password prompt, but it does raise the system "security warning" dialog (click 是(Y)); uninstalling is `certutil -user -delstore Root <fingerprint>` and raises a "根证书存储" confirmation the same way. Both steps wait for a human click, so the app allows 120 s for them (`KI-021`).
...
- The full acceptance procedure (including the registrar browser acceptance and result table) is in [specs/001-phase1-local-bridge/verification.md](../../specs/001-phase1-local-bridge/verification.md), section "M4 双平台验收执行手册"; the measured Windows run is the section "M4 Windows 验收执行记录（2026-09-23）" of the same file. If a report shows `FAIL bridge-smoke 上游 502（KI-019 有界失败）`, the upstream connect failed **within its bound** (4 s × 2 attempts; a timeout reaches the client as `502 Bad Gateway`) rather than curl waiting forever — determine the cause with the stage re-run below instead of reading it as a bridge failure.

### Stage re-run for bridged stalls (`KI-019`)

With the app running, the bridge "running", the system proxy enabled and a valid session:

```bash
pnpm run diagnose:upstream --user-data-dir <profile> --rounds 40 --scheme http
```

- Each round follows a fixed rhythm: DNS → TCP → TLS → the bridged request → DNS/TCP/TLS again → same-path direct control. Rounds print a `no-stall` / `bridge-side` / `network-or-resolver` verdict, and abnormal rounds additionally print the matching stage records from `<userData>/bridge-upstream.log` plus a live `Get-NetRoute` reading. Evidence goes to `<out>/diagnose-upstream-<platform>-<timestamp>.jsonl` (`--out` defaults to the system temp directory and **must not** point inside the repository).
- The direct control carries **no cookie**: the gateway answers a cookie-less WRD request with `302 → https://<gateway>/login` (and hands out a fresh ticket), so the control measures **path health only** (DNS/TCP/TLS/first byte) and is not a session signal by itself. Session expiry is read from the **bridged** response (only a bridged `302 → gateway login page` counts as expired and stops the run); a bridged request without response headers (`(无状态行)`) is a stall sample and does not abort.
- The script stops with a clear error — and no conclusion — when the bridge is not running (port unreachable in three consecutive rounds), the session has expired (bridged `302 → gateway login page`), or `bridge-config.json` / the CA is missing from `--user-data-dir`.
- The re-run environment must match the historical failure rounds: a direct default route with no TUN route (the `0.0.0.0/0` row of `Get-NetRoute -AddressFamily IPv4`).
- Criterion: `bridge-side` = that round's bridged request was abnormal while the same-round direct control (DNS/TCP/TLS) was healthy; `network-or-resolver` = the same-round direct control was unhealthy too, or DNS took ≥ 2 s. Update `KI-019` on that basis.
...
| Login window styling broken / overlapping pop-ups | CAS theme assets are intermittently truncated on a transfer path outside the app and bridge (`ERR_INCOMPLETE_CHUNKED_ENCODING`, `KI-014`) | Close the login window and click "log in to WebVPN" again to reload it; retain this workaround until the external transfer path stabilises |
...
| After clicking "install / uninstall local CA" the UI reports "命令超时（120000ms）：certutil" (Windows) | The OS dialog itself waits for a human: "安全警告" before the certificate is written (it lists the fingerprint, 是(Y) / 否(N)) and a "根证书存储" confirmation before it is deleted; `certutil` blocks meanwhile and the app now waits 120 s (`KI-021`; fixed 2026-09-23 — the step used the default 10 s before, so the process was killed while the user was still reading) | Just click 是(Y) when the dialog appears (valid for two minutes); if it really did time out, run the command by hand — install: `certutil -user -addstore Root <userData>\mitmproxy\mitmproxy-ca-cert.cer`, uninstall: `certutil -user -delstore Root <fingerprint>` (both raise the same dialogs; no administrator rights needed) — then the UI refreshes |
| The registrar page shows nothing for tens of seconds on first open | Sporadic bridged-request stall (`KI-019`): the bridge now writes each upstream stage's timing and peer address to `<userData>/bridge-upstream.log` (`swufe-upstream` records), which separates "inside the bridge's connect", "connected but the gateway never answers" and "local network/resolver" | Reload the page first (a reload used to fix it). Then read the log: `connect_timeout` / `connect_retry` / `connect_failed` ⇒ the connect bound is in force (the client gets `502` within 8 s instead of waiting); connect stages present but no `response` with `error` = `client disconnected` ⇒ the gateway is connected but silent (that stage cannot be interrupted, see `docs/api/bridge-control-protocol.md`), so reload/reopen; if it still reproduces, run `pnpm run diagnose:upstream` for a same-round direct control before concluding |
...
> Windows real-machine verification (TC-C02..C04, TC-D01..D04, TC-E01/E02/E03, TC-F01/F04, TC-G03/G04, TC-H01/H02, TC-B05) ran on **2026-09-23** on Windows 11 24H2 and passed; it surfaced `KI-015`..`KI-018`, all fixed, plus `KI-020` (tooling platform assumptions) — see [known-issues.md](../../specs/001-phase1-local-bridge/known-issues.md). The run and its command results are in the "M4 Windows 验收执行记录（2026-09-23）" section of [verification.md](../../specs/001-phase1-local-bridge/verification.md). `KI-019` is provisionally `Accepted`: the latest test observed no inaccessibility through either the bridge or direct access; its cause remains unknown and the issue reopens on recurrence.
