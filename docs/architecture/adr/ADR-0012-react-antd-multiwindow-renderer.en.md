...
- The renderer must be **fully offline**: the current CSP is `default-src 'none'; script-src 'self'; style-src 'self'; img-src 'self' data:`, so no CDN and no remote fonts/icons ([security/README.md](../../security/README.md) TB-003, and the [CSP conventions are fixed in the implementation](../../../apps/desktop/vite.config.ts)).
