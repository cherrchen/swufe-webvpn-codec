...
| Telemetry UI | In-process module (Electron Renderer) | Capture-mode selection, allowlist editing, status display and the debug log panel | `apps/desktop/src/renderer/renderer.ts`, `apps/desktop/static/{index.html,styles.css}`, `apps/desktop/src/preload/index.ts` | Implemented (M3: status bar, bridge switch, certificate, proxy status, debug switch + the capture-mode section, allowlist editing and the log panel) |
