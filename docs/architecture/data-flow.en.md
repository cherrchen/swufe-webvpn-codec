...
| DF-005 | System proxy set and clear | Start bridge / stop bridge / session expiry / exit | Current OS proxy settings | Read the OS proxy → if enabled and not this bridge, refuse to start (`PROXY_CONFLICT`) → otherwise set `127.0.0.1:<bridge_port>` and record the "set by this app" marker → clear only when the marker exists | System proxy pointing at the bridge, or restored | `AppSettings.systemProxyManagedByApp` (runtime marker) | [components.md](components.md), [api/electron-ipc.md](../api/electron-ipc.md) |
...
| Storage | `userData/config.json` (settings + allowlist), `userData/session.bin` (encrypted) or an Electron persistent session partition, CA in the mitmproxy-specific confdir | Cookies live in the user directory with tightened permissions; the CA private key stays local |
| Use / derivation | DF-001/DF-002 use the Cookies and allowlist; DF-005 uses the "set by this app" marker | Session and marker are only valid while the bridge runs; the marker expires with the clearing action |
