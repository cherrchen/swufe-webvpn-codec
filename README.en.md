...
| Item | Value |
| --- | --- |
| Stage | Phase 1 real-machine acceptance passes on macOS and Windows, including browser interaction with the academic-affairs site; spec 001 is `Implemented`. `KI-014` is accepted as asset truncation outside the app, with login-window reload as a workaround. The local versus external cause of the sporadic bridged-request stall `KI-019` remains unresolved, so the spec is not yet `Verified`. See [verification.md](specs/001-phase1-local-bridge/verification.md). |
